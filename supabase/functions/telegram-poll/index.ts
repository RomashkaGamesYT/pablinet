      ...
      if (text === "/start") {
        await sendTelegramMessage(
          ...
          "👋 Привет! Я бот соцсети нэт.\n\n" +
            ...
            "📱 Если ты входишь по номеру телефона — перейди по ссылке из приложения.(временно не работает)\n\n" +
            "Например: @myusername",
          LOVABLE_API_KEY,
          TELEGRAM_API_KEY,
        );
        ...
        if (!username) {
          await sendTelegramMessage(
            chatId,
            "❌ Укажи username. Например: @myusername",
            LOVABLE_API_KEY,
            TELEGRAM_API_KEY,
          );
          continue;
        ...
        if (!profile) {
          await sendTelegramMessage(
            chatId,
            `❌ Пользователь @${username} не найден в соцсети нэт.`,
            LOVABLE_API_KEY,
            TELEGRAM_API_KEY,
          );
          continue;
        ...
        const { data: flameBadge } = await supabase.from("badges").select("id").eq("name", "Flame").single();
        ...
        if (flameBadge) {
          ...
          if (hasBadge) {
            await sendTelegramMessage(
              chatId,
              "✅ У пользователя @" + username + " уже есть бейдж Flame!",
              LOVABLE_API_KEY,
              TELEGRAM_API_KEY,
            );
            continue;
...
async function handleVerifyToken(
  ...
  if (error || !authCode) {
    await sendTelegramMessage(
      chatId,
      "❌ Ссылка недействительна или истекла. Запросите новый код.",
      lovableKey,
      telegramKey,
    );
    return;
